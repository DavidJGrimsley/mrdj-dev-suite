...
import { getPersistedOnboardingState, setPersistedOnboardingState } from './onboarding-store';
...
export function createZustandLegalAcceptanceAdapter(): MemoryLegalAcceptanceAdapter {
  return {
    ...
    async acceptLegalDocument(document, input) {
      const next = upsertLegalAcceptance(
        getPersistedOnboardingState() ?? createEmptyOnboardingState(),
        {
