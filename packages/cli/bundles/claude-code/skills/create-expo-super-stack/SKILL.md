---
description: Create either one Expo app or a multi-app workspace with the MDS Super Stack flow. Use callable MDS MCP tools as the guided intake surface and the Super Stack generator as the scaffold source of truth.
...
- For a workspace generated by this flow, confirm the root has `project/workspace.json` plus the canonical root project-memory files, and each selected app has its own canonical project-memory files. The separate workspace control plane uses `project/mds.workspace.json` when applicable; do not substitute one manifest for the other or recreate obsolete intake/evidence files or a tracked worktree registry.
