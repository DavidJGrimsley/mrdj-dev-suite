...
describe('Experimemo generator fixture regression', () => {
  it('keeps generated Link and modal patterns safe', async () => {
    ...
    const layout = await readFile(path.join(fixturePath, 'src', 'app', '_layout.tsx'), 'utf8');
    const settings = await readFile(
      path.join(fixturePath, 'src', 'features', 'settings', 'settings-screen.tsx'),
      'utf8'
    );
    const headerButton = await readFile(
      path.join(fixturePath, 'src', 'components', 'HeaderButton.tsx'),
      'utf8'
    );
    const appJson = JSON.parse(await readFile(path.join(fixturePath, 'app.json'), 'utf8')) as {
    ...
    expect(layout).toContain('NavigationBar.setStyle(');
    expect(layout).not.toContain('<NavigationBar');
    expect(settings).toContain("Platform.OS === 'web'");
    expect(settings).toContain('styles.webOverlay');
    expect(headerButton).toContain('<Pressable ref={ref}');
  ...
  it('ejects Stylist and exposition safely from a disposable fixture copy', async () => {
    ...
    await expect(
      readFile(path.join(copyPath, 'src', 'features', 'exposition', 'stylist-screen.tsx'), 'utf8')
    ).rejects.toThrow();
    await expect(
      readFile(
        path.join(copyPath, 'src', 'features', 'exposition', 'exposition-screen.tsx'),
        'utf8'
      )
    ).rejects.toThrow();
    await expect(
      readFile(path.join(copyPath, 'src', 'features', 'settings', 'settings-screen.tsx'), 'utf8')
    ).resolves.toContain('Settings');

    const layout = await readFile(path.join(copyPath, 'src', 'app', '_layout.tsx'), 'utf8');
    expect(layout).toMatch(/<Stack\.Screen\s+name="settings"/u);
    expect(layout).toContain("presentation: 'modal'");
...
async function copyFixtureToTemp(): Promise<string> {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'mds-experimemo-fixture-'));
...
async function findRiskyAsChildStyleArrayFiles(sourceDir: string): Promise<string[]> {
...
function findRiskyAsChildStyleArrayBlocks(source: string): string[] {
  ...
  const linkBlocks = source.matchAll(/<Link\b(?=[^>]*\basChild\b)[^>]*>[\s\S]*?<\/Link>/gu);
  for (const match of linkBlocks) {
    const block = match[0];
    if (/<(?:Pressable|TouchableOpacity|TouchableHighlight)\b[^>]*style=\{\[/u.test(block)) {
