...
- [x] Dogfood on Time2Pay, PokePages and the Dogfood app.
...
- [x] Week 1: repo, Turborepo, shared package, knowledge package, skills markdown, docs folder, and initial MCP server skeleton.
- [x] Week 2: Doctor v0 with app file size, business logic keywords, SSR globals, public secret vars, SEO basics, and Expo config basics.
- [x] Week 3: MCP tools for project/file scans, skills/guides, and project reports.
- [x] Week 4: CLI installer with `mds init`, `mds doctor`, `mds mcp install`, `mds codex install`, and `mds claude install`.
- [x] Week 5: Codex and Claude bundles with shared skills, commands, MCP config, and install docs.
- [x] Week 6: real-world dogfood with reports, fixes, examples, and roadmap cleanup.
...
## Workspace initialization hardening

- [ ] In the Infie initialization intake, ask the UD for the workspace parent folder before apply; default beside the source only when the UD does not choose a shared parent.
- [ ] Before apply, ask the UD whether to remove confirmed-merged worktrees, retain all worktrees, or also remove unverified/no-PR worktrees. Recommend removing only confirmed-merged checkouts while retaining open and unknown work.
- [ ] Keep non-interactive `--yes` non-destructive: worktree cleanup must require an explicit cleanup option.
- [ ] Offer opt-in relocation of auxiliary directories such as `test-apps`, while keeping the generic workspace model free of MDS-specific paths.
- [ ] Add lock preflight plus a durable operation journal, resumable apply, and rollback so a lock error cannot leave a half-moved workspace.
- [ ] Create required root folders before irreversible moves and make already-normalized partial workspaces safely resumable.
- [ ] Never run dependency installation as part of workspace initialization or recovery.
- [ ] Surface control-repository creation, privacy, and push as explicit planned side effects before apply.
- [ ] Add an explicit, reviewable legacy-project consolidation workflow: collect differences from every linked worktree's tracked `project/` directory into the control project, require human resolution for conflicts, then remove legacy source copies only in a normal source-repository PR.
- [ ] Make workspace status surface clean-but-behind main as a prompt for Infie to offer a safe fast-forward; do not update it without UD approval.
- [ ] Implement the control-repo synchronization workflow: one canonical human-readable `project/todo.md`, coordinator-owned minimal edits, agent-private structured state outside Markdown, and branch/push/PR updates for approved control-plane changes from Infie, I² web, and I² app clients.
