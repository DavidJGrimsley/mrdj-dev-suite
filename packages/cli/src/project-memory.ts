...
export function renderGuidelines(answers: OnboardAnswers): string {
  return [
    ...
    '- In an initialized MDS workspace, resolve the workspace control repository with `mds workspace discover .`; its root `project/` folder is canonical and checkout-local legacy `project/` folders are read-only migration inputs.',
...
export function renderAgentInstructions(answers: OnboardAnswers): string {
  return [
    ...
    '',
    'When `.mds/workspace.json` exists, first run `mds workspace discover .` and read the discovered control repository project memory instead of a checkout-local legacy copy.',
    '',
    '- `project/info.md`',
    '- `project/todo.md`',
    '- `project/style.md`',
...
function renderRichRootLayout(
  ...
  return [
    ...
    '            {Platform.OS === "android" ? (',
    '              <NavigationBar',
    '                style={theme.activeScheme === "dark" ? "dark" : "light"}',
    '              />',
    '            ) : null}',
    '            <Stack',
    '              screenOptions={{',
...
function renderExpositionScreen(includeNativeWindUiExposition = false): string {
  const nativeWindUiRouteCard = includeNativeWindUiExposition
    ? [
        ...
        '          <Text style={[styles.link, { color: colors.secondary }]}>Open NativeWindUI screen</Text>',
        '        </Link>',
        '      </PackageCard>',
      ]
    : [];
  ...
  return [
    ...
    '      >',
    "        <Text style={[styles.link, { color: colors.secondary }]} onPress={() => Linking.openURL('https://github.com/alabsi91/reanimated-color-picker')}>",
    ...
    '      >',
    "        <Text style={[styles.link, { color: colors.secondary }]} onPress={() => Linking.openURL('https://react-native-async-storage.github.io/async-storage/')}>",
    '          Async Storage Docs',
    '        </Text>',
    '      </PackageCard>',
    '      <PackageCard',
    ...
    '      >',
    "        <Text style={[styles.link, { color: colors.secondary }]} onPress={() => Linking.openURL('https://docs.expo.dev/versions/latest/sdk/safe-area-context/')}>",
    ...
    '      >',
    "        <Text style={[styles.link, { color: colors.secondary }]} onPress={() => Linking.openURL('https://tailwindcss.com/docs/customizing-colors')}>",
    '          Tailwind CSS - Colors',
    '        </Text>',
    '      </PackageCard>',
    '      <PackageCard',
    ...
    '      >',
    "        <Text style={[styles.link, { color: colors.secondary }]} onPress={() => Linking.openURL('https://docs.expo.dev/router/web/api-routes/')}>",
    ...
    "        <Text style={[styles.link, { color: colors.secondary }]} onPress={() => Linking.openURL('https://docs.swmansion.com/react-native-reanimated')}>",
    ...
    "        <Text style={[styles.link, { color: colors.secondary }]} onPress={() => Linking.openURL('https://docs.swmansion.com/react-native-gesture-handler')}>",
    ...
    '      >',
    '        <ScreensCard />',
    "        <Text style={[styles.link, { color: colors.secondary }]} onPress={() => Linking.openURL('https://docs.swmansion.com/react-native-screens')}>",
    ...
    "        <Text style={[styles.link, { color: colors.secondary }]} onPress={() => Linking.openURL('https://docs.expo.dev/versions/latest/sdk/svg')}>",
    '          Expo SDK - SVG',
    '        </Text>',
    '      </PackageCard>',
    '      <PackageCard',
    ...
    '      >',
    '        <KeyboardForm />',
    "        <Text style={[styles.link, { color: colors.secondary }]} onPress={() => Linking.openURL('https://kirillzyusko.github.io/react-native-keyboard-controller/')}>",
...
function renderExpoSdk56Screen(answers: OnboardAnswers): string {
  const expoUiDemo = answers.usesExpoUiUniversalComponents
    ? [
        ...
        '  return (',
        '    <View style={[styles.exampleBox, { backgroundColor: colors.surface, borderColor: colors.primary }]}>',
    ...
    : [
        ...
        '  return (',
        '    <View style={[styles.exampleBox, { backgroundColor: colors.surface, borderColor: colors.primary }]}>',
        '      <Text style={[styles.exampleTitle, { color: colors.text }]}>Universal components are not enabled.</Text>',
        '      <Text style={[styles.exampleBody, { color: colors.text }]}>Turn on Expo UI Universal in onboarding to generate a Host, Column, Text, Button, and Switch demo here.</Text>',
        '    </View>',
        '  );',
        '}',
        '',
  ...
  return [
    ...
    '  const theme = useAppTheme();',
    '  const colors = theme.activeColors;',
    '  const boxStyle = [styles.exampleBox, { backgroundColor: colors.surface, borderColor: colors.primary }];',
    '  const titleStyle = [styles.exampleTitle, { color: colors.text }];',
    '  const bodyStyle = [styles.exampleBody, { color: colors.text }];',
    '  const pillStyle = [styles.examplePill, { backgroundColor: colors.background, borderColor: colors.primary, color: colors.text }];',
    '  const labelStyle = [styles.componentLabel, { backgroundColor: colors.primary, color: colors.background }];',
    '  if (kind === "universal") return <UniversalPreview />;',
    '  if (kind === "expo-ui") {',
    '    return (',
    '      <View style={boxStyle}>',
    '        <Text style={titleStyle}>Native controls from one React surface</Text>',
    '        <View style={styles.exampleRow}><Text style={pillStyle}>SwiftUI</Text><Text style={bodyStyle}>iOS controls render with native behavior.</Text></View>',
    '        <View style={styles.exampleRow}><Text style={pillStyle}>Compose</Text><Text style={bodyStyle}>Android controls stay platform-native.</Text></View>',
    '      </View>',
    '    );',
    '  }',
    ...
    '        <View style={styles.exampleRow}><Text style={pillStyle}>Slider</Text><Text style={bodyStyle}>Use the Expo UI version where native fidelity matters.</Text></View>',
    '        <View style={styles.exampleRow}><Text style={pillStyle}>Picker</Text><Text style={bodyStyle}>Swap community picker screens one at a time.</Text></View>',
    '      </View>',
    '    );',
    '  }',
    ...
    '        <View style={styles.componentLabelGrid}><Text style={labelStyle}>RN 0.85</Text><Text style={labelStyle}>React 19.2</Text><Text style={labelStyle}>Hermes V1</Text></View>',
    '      </View>',
    '    );',
    '  }',
    '  if (kind === "widgets") {',
    '    return (',
    '      <View style={boxStyle}>',
    '        <View style={[styles.widgetTile, { backgroundColor: colors.background, borderColor: colors.primary }]}><Text style={[styles.widgetTitle, { color: colors.text }]}>Today</Text><Text style={[styles.widgetBody, { color: colors.text }]}>3 tasks ready</Text></View>',
    '      </View>',
    '    );',
    '  }',
    '  return (',
    ...
    '      <View style={styles.transportRow}><Text style={[styles.transportButton, { backgroundColor: colors.background, borderColor: colors.primary, color: colors.text }]}>Play</Text><Text style={[styles.transportButton, { backgroundColor: colors.background, borderColor: colors.primary, color: colors.text }]}>Pause</Text><Text style={[styles.transportButton, { backgroundColor: colors.background, borderColor: colors.primary, color: colors.text }]}>Haptic tap</Text></View>',
    '    </View>',
    '  );',
    '}',
    '',
    ...
    '      <Text style={[styles.intro, { color: colors.text }]}>Review the SDK 56 changes before deciding what belongs in the real app.</Text>',
    '      <ExpositionNotice />',
    '      {highlights.map((item) => (',
    ...
    '                {item.links.map((link) => (',
    '                  <Text key={link.href} accessibilityRole="link" onPress={() => Linking.openURL(link.href)} style={[styles.link, { color: colors.secondary }]}>',
    '                    {link.label}',
    '                  </Text>',
    '                ))}',
    '              </View>',
...
function renderDataScreen(answers: OnboardAnswers): string {
  ...
  return [
    ...
    '      <ExpositionNotice />',
    '      <Pressable onPress={addTask} style={[styles.button, { backgroundColor: colors.secondary, borderRadius: theme.layout.radius }]}>',
...
function renderSupabaseDataScreen(answers: OnboardAnswers): string {
  return [
    ...
    '              ]}>',
    '              <Text style={[styles.buttonText, { color: secondaryForeground }]}>{isSubmitting ? "Signing..." : "Sign guestbook"}</Text>',
    '            </Pressable>',
    '          </>',
    '        )}',
    '      </View>',
