...
export function renderGuidelines(answers: OnboardAnswers): string {
  return [
    ...
    '- Only proceed when zero markers remain. `mrdj doctor` also surfaces this as a warning.',
    '',
    '## Expo Architecture',
    '',
...
export function renderAgentInstructions(answers: OnboardAnswers): string {
  return [
    ...
    '',
    'Before any intake, planning, or scaffolding, scan every `project/` file for the marker `# TodoForContext(optional):`. If any are present, stop and tell the user to fill the section underneath OR delete the marker line. Only proceed when zero markers remain.',
    '',
